...
(function () {
  ...
  if (DEBUG) console.info('[TVS] Activity saved:', res);
    ...
    if (!data) {
      return h(
        'div',
        { className: 'tvs-loading', style: { display: 'flex', alignItems: 'center', gap: '8px', color: '#666', padding: '0.5rem 0' } },
        h('span', { className: 'tvs-spinner', 'aria-hidden': 'true' }),
        h('span', null, 'Loading...')
      );
    }
          ...
          ? (
              (currentTime > 0 && sessionStartAt && (duration === 0 || currentTime < duration - 0.5))
                ? [
                    h(
                      "button",
                      { onClick: resumeActivitySession, disabled: isPosting },
                      isPosting ? h("span", { className: "tvs-spinner", "aria-hidden": "true" }) : null,
                      isPosting ? " Resuming..." : "Resume Activity"
                    )
                  ]
                : [
                    h(
                      "button",
                      { onClick: startActivitySession, disabled: isPosting },
                      isPosting ? h("span", { className: "tvs-spinner", "aria-hidden": "true" }) : null,
                      isPosting ? " Starting..." : "Start Activity"
                    )
                  ]
