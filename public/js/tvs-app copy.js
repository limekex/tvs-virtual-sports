...
(function () {
  ...
  const DEBUG = new URLSearchParams(location.search).get("tvsdebug")==="1"
...
if (!DEBUG) {
  ...
  // WordPress tilbyr React via wp.element. På noen oppsett finnes også window.React/ReactDOM.
  ...
  function mountReact(Component, props, node) {
    try {
      if (hasCreateRoot) {
        const createRoot =
          (ReactDOM && ReactDOM.createRoot) || wpEl.createRoot;
      ...
      const legacyRender =
        (ReactDOM && ReactDOM.render) || (wpEl && wpEl.render);
      if (legacyRender) {
  ...
  function DevOverlay({ data }) {
    ...
    const title = (window.TVS_SETTINGS && window.TVS_SETTINGS.env) || "n/a";
    const rid = data?.id || "n/a";
    const payload = !!window.tvs_route_payload;
    return React.createElement(
      ...
      React.createElement(
        ...
        String(payload),
        " | createRoot: ",
        String(hasCreateRoot)
      ),
      React.createElement(
        "div",
        { style: { marginTop: 6, opacity: 0.9 } },
        "Tips: legg til ?tvsdebug=1 i URL for å aktivere logging."
      )
    );
  }
function Loading() {
  ...
  return h(
    ...
    h(
      ...
      // Track (bakken)
      h("line", { x1: 4, y1: 60, x2: 60, y2: 60, stroke: "#bbb", "stroke-width": 2, className: "track" }),
      // Hode
      h("circle", { cx: 26, cy: 12, r: 5, fill: "none", stroke: "#111", "stroke-width": 2 }),
      // Kropp
      h("line", { x1: 26, y1: 17, x2: 26, y2: 35, stroke: "#111", "stroke-width": 2 }),
      // Armer
      h("line", { x1: 26, y1: 22, x2: 40, y2: 18, stroke: "#111", "stroke-width": 2, className: "arm front", style: { transformOrigin: "26px 22px" } }),
      h("line", { x1: 26, y1: 22, x2: 12, y2: 26, stroke: "#111", "stroke-width": 2, className: "arm back",  style: { transformOrigin: "26px 22px" } }),
      // Bein
      h("line", { x1: 26, y1: 35, x2: 40, y2: 48, stroke: "#111", "stroke-width": 2, className: "leg front", style: { transformOrigin: "26px 35px" } }),
      h("line", { x1: 26, y1: 35, x2: 16, y2: 54, stroke: "#111", "stroke-width": 2, className: "leg back",  style: { transformOrigin: "26px 35px" } })
    ),
    // Skeleton–tekst
    h(
      "div",
  ...
  return h(
    ...
    h("div", { className: "tvs-dev__body" },
      ...
      h("div", { className: "tvs-dev__actions" },
        ...
        h("button", {
          onClick: () => { localStorage.setItem('tvsDev','0'); location.reload(); },
  ...
  function App({ initialData, routeId }) {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const r = await fetch(`/wp-json/tvs/v1/routes/${encodeURIComponent(routeId)}`, { credentials: "same-origin" });
            ...
            setLastError(e?.message||String(e)); setLastStatus('error'); }    
            }
        })();
        }, [routeId]);
    ...
    return h(
      ...
      h(
        "div",
        { className: "tvs-meta" },
        h("pre", null, JSON.stringify(meta, null, 2))
      ),
      h(
        "button",
